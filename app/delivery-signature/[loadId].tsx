/**
 * Delivery Signature Screen
 *
 * Shown when the driver taps "Mark as Delivered" (after delivery photos).
 * Three paths:
 *   1. Customer is present → draw customer signature → draw driver signature → confirm immediately
 *   2. Customer not available → draw driver signature → confirm immediately (or auto-confirm if saved sig)
 *   3. Skip signature → confirm with warning
 *
 * On confirmation the screen updates local load status to "delivered",
 * saves signatures to the server, fires the platform sync mutation,
 * then navigates back to the loads list.
 */
...
export default function DeliverySignatureScreen() {
  ...
  const doConfirmDelivery = useCallback((overrideDriverSig?: string, overrideCustomerNotAvailable?: boolean) => {
  ...
  const handleDriverSigConfirm = useCallback(() => {
    ...
    doConfirmDelivery();
  }, [hasDriverSig, isConfirming, hasSavedDriverSig, driverPaths, setDriverSignaturePaths, doConfirmDelivery]);
  ...
  const headerTitle =
    mode === "choose" ? "Delivery Signature" :
    ...
    <View style={{ flex: 1, backgroundColor: colors.background }}>
      ...
      >
        ...
        {mode === "choose" && (
          <View>
            <Text style={[styles.sectionDesc, { color: colors.muted }]}>
              Select how you would like to capture the delivery confirmation for{" "}
            ...
            <OptionButton
              ...
              onPress={() => {
                ...
                if (hasSavedDriverSig) {
                  ...
                  doConfirmDelivery(savedSig, true);
                } else {
            ...
            <OptionButton
              ...
              onPress={() => {
                ...
                Alert.alert(
                  "Skip Signature?",
                  "Are you sure you want to proceed without capturing any signature? This may affect your proof of delivery.",
                  [
                    { text: "Cancel", style: "cancel" },
                    { text: "Skip", style: "destructive", onPress: () => doConfirmDelivery() },
                  ]
                );
              }}
              color={colors.muted}
        ...
        {mode === "customer_sig" && (
          <View>
            <Text style={[styles.sectionDesc, { color: colors.muted }]}>
              Hand the device to the customer and ask them to sign to confirm receipt of their vehicle(s).
            </Text>
            {/* Customer Name Field */}
        ...
        {mode === "driver_sig" && (
          <View>
            ...
            <Text style={[styles.sectionDesc, { color: colors.muted }]}>
              {customerNotAvailable
                ? "Sign below to confirm you delivered the vehicle(s) without the customer present."
                : "Now sign below as the driver to complete the delivery confirmation."}
            </Text>
            <SignaturePad
